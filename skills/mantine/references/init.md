# Mantine Initialization & Setup

Follow these steps sequentially when asked to setup or initialize Mantine:

1. **Ask User for Packages**: Use the `ask_question` tool (or other similar tools to get multi-choice answer from the user) to determine which Mantine packages the user wants to install.
...
2. **Determine Package Manager**: Detect the package manager used in the active project by checking the root directory for lockfiles.
   ...
   - If no lockfile is found, use pnpm.

3. **Install Packages**: Execute the installation command for the packages selected by the user. If the current project is part of a monorepo then install the packages within the particular project, not in the root dir.
   - **CRITICAL INSTRUCTION**: Under no circumstances manually add packages in `package.json`. You must install/update packages by using `pnpm add <packagename>@latest` or `pnpm add <packagename>@<specific-version>` when necessary. Of course, use the relevant package manager according to the current project.
